                         ...
                         int flag	/*!< either 0 for normal operation, or finite timestep to get corresponding aphys */ )
{
	...
	if(dt > 0)
	  {
            double p_target = 0.02; // desired maximum probability per timestep
            ...
            double p_dt = prob_of_interaction_robertson(P[p].Mass,0.,PPP[p].AGS_Hsml,dV,dt); // probability of interacting with another DM particle well within kernel, assuming same mass, H, and V~signalvel, for current timestep dt
#else
	    double p_dt = prob_of_interaction(P[p].Mass,0.,PPP[p].AGS_Hsml,dV,dt);
#endif
