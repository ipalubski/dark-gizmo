...
{
#if defined(DM_SIDM)
  double sidm_kick[3], dtime_sidm, rholoc; int si_count;
#endif
#ifdef DM_FUZZY
...
int AGSForce_evaluate(int target, int mode, int *exportflag, int *exportnodecount, int *exportindex, int *ngblist, int loop_iteration)
{
    ...
    while(startnode >= 0)
    {
		...
		r2 = kernel.dp[0]*kernel.dp[0] + kernel.dp[1]*kernel.dp[1] + kernel.dp[2]*kernel.dp[2];
		if(r2>thresh_val) {thresh_val=r2;}
		
	      }
	    out.rholoc = numngb_inbox*P[j].Mass/(4.0/3.0*M_PI*pow(thresh_val,3));
	    
	    //printf("ngh # = %i, rmax = %f, r_i = %f, h = %f \n",numngb_inbox,thresh_val,sqrt(local.Pos[0]*local.Pos[0]+local.Pos[1]*local.Pos[1]+local.Pos[2]*local.Pos[2]),local.AGS_Hsml);
	    if(numngb_inbox = 0 ){out.rholoc = 0;}
	    //kernel.DMRho = out.rholoc;
