      ...
      /* do check on whether we have enough neighbors, and iterate for density-hsml solution */
        ...
        for(i = FirstActiveParticle, npleft = 0; i >= 0; i = NextActiveParticle[i])
        {
            if(ags_density_isactive(i))
            {
                ...
                if(ConditionNumber > MAX_REAL_NUMBER) {PRINT_WARNING("CNUM for CBE: ThisTask=%d i=%d ConditionNumber=%g desnumngb=%g NumNgb=%g iter=%d NVT=%g/%g/%g/%g/%g/%g AGS_Hsml=%g \n",ThisTask,i,ConditionNumber,desnumngb,PPP[i].NumNgbwPPP[i].NumNgb,iter,P[i].NV_T[0][0],P[i].NV_T[1][1],P[i].NV_T[2][2],P[i].NV_T[0][1],P[i].NV_T[0][2],P[i].NV_T[1][2],PPP[i].AGS_Hsml);}
...
#endif
#ifdef DM_SIDM
    if((1 << P[i].Type) & (DM_SIDM)) {default_to_return = 0;}
#endif
...
{
  double dp[3], dv[3], r, wk_i, wk_j, dwk_i, dwk_j, h_i, hinv_i, hinv3_i, hinv4_i, h_j, hinv_j, hinv3_j, hinv4_j; int  ngbcount;
};
...
int cmp(const void *a, const void *b)
{
  ...
  if ((*a1).Rngb2 < (*a2).Rngb2)
    return -1;
  else if ((*a1).Rngb2 > (*a2).Rngb2)
    return 1;
  else
    return 0;
}
...
{
#if defined(DM_SIDM)
  double sidm_kick[3], dtime_sidm, rholoc; int si_count, ngbcount;
#endif
#ifdef DM_FUZZY
...
static inline void OUTPUTFUNCTION_NAME(struct OUTPUT_STRUCT_NAME *out, int i, int mode, int loop_iteration)
{
    ...
    P[i].rholoc = out->rholoc;
    P[i].ngbcount = out->ngbcount;
#endif
#ifdef DM_FUZZY
...
int AGSForce_evaluate(int target, int mode, int *exportflag, int *exportnodecount, int *exportindex, int *ngblist, int loop_iteration)
{
    ...
    int ngbcount;
    //int tol = 5;
    double* rngb_node = (double *)malloc(10 * sizeof(double));
    
    /* Find the appropriate search length first (need at least 32 neighbors) */
    ...
    //	  }
    /*	if(ngbcount > 32){LOOKING = false;}
	if(searchiter > 100){LOOKING = false;}
	double rr = sqrt(local.Pos[0]*local.Pos[0] + local.Pos[1]*local.Pos[1] + local.Pos[2]*local.Pos[2]);
	//printf("Found %i ngbs, r = %f, search_len = %f, ID = %i\n",ngbcount,rr,search_len,local.ID);
	search_len *= 1.1;
	kernel.h_i *= 1.1;kernel.hinv_i=1/kernel.h_i;kernel.hinv3_i=kernel.hinv_i*kernel.hinv_i*kernel.hinv_i;kernel.hinv4_i=kernel.hinv3_i*kernel.hinv_i;
	searchiter += 1;
      }*/
    //if(searchiter > 100){return 0;}
    //if(ngbcount < 32){printf("NGB SEARCH ERROR #%i",ngbcount);}
    //printf("Found %i ngbs, ID = %f\n",ngbcount,local.ID);
    //return 0;
    //printf("CALC DIST\n");
    ...
    if(mode == 0) {startnode = All.MaxPart; /* root node */} else {startnode = DATAGET_NAME[target].NodeList[0]; startnode = Nodes[startnode].u.d.nextnode;    /* open it */}
    ngbcount=0;
    ///memset(rngb, 0, 100*sizeof(double));
    ///memset(ngblistsum, 0, 100*sizeof(int));
    while(startnode >= 0)
      {
	while(startnode >= 0)
	  {
	    numngb_inbox = ngb_treefind_variable_threads_targeted(local.Pos, search_len, target, &startnode, mode, exportflag, exportnodecount, exportindex, ngblist, AGS_kernel_shared_BITFLAG);
	    if(numngb_inbox <= 0) {return -2;} /* no neighbors! */
	    //rngb_node = realloc(rngb_node,numngb_inbox * sizeof(double));
	    for(n = 0; n < numngb_inbox; n++)
	      {
		j = ngblist[n];
		if((P[j].Mass <= 0)||(PPP[j].AGS_Hsml <= 0)) continue;
		if(((1 << local.Type) & (DM_SIDM)) && ((1 << P[j].Type) & (DM_SIDM)) && (local.ID != P[j].ID))
		  {
		    kernel.dp[0] = local.Pos[0] - P[j].Pos[0]; kernel.dp[1] = local.Pos[1] - P[j].Pos[1]; kernel.dp[2] = local.Pos[2] - P[j].Pos[2];
		    NEAREST_XYZ(kernel.dp[0],kernel.dp[1],kernel.dp[2],1);
		    r2 = kernel.dp[0]*kernel.dp[0] + kernel.dp[1]*kernel.dp[1] + kernel.dp[2]*kernel.dp[2];
		    //double h2 = kernel.h_i*kernel.h_i;
		    //kernel.r = sqrt(r2);
		    //rngb_node[n] = sqrt(r2);
		    ///double u = kernel.r * kernel.hinv_i;
		    ///kernel_main(u, kernel.hinv3_i, kernel.hinv4_i, &kernel.wk_i, &kernel.dwk_i, 0);
		    ///double mj_wk = FLT(local.Mass * kernel.wk_i);
		    ///out.rholoc += mj_wk; //pow(kernel.h_i,3);
		  }
	      }
	    ///printf("appending ");
	    //memcpy(rngb+ngbcount,rngb_node,numngb_inbox*sizeof(double)); // Append the distance array for this node
	    //ngbcount += numngb_inbox; // Add neighbors on this leaf to the total count
	    //out.ngbcount += numngb_inbox;
	    //memcpy(ngblistsum+ngbcount,ngblist,numngb_inbox*sizeof(int));
	    ///printf(" appended!\n");
	  }
	//if(ngbcount > 32){LOOKING = false;}//printf("ENDED BY NGBCOUNT\n");}
	//if(searchiter > 100){LOOKING = false;}//printf("ENDED BY ITER\n");}
	double rr = sqrt(local.Pos[0]*local.Pos[0] + local.Pos[1]*local.Pos[1] + local.Pos[2]*local.Pos[2]);
	//printf("Found %i ngbs, r = %f, search_len = %f, ID = %i\n",ngbcount,rr,search_len,local.ID);                                                    
	//search_len *= 1.1;
	//kernel.h_i *= 1.1;kernel.hinv_i=1/kernel.h_i;kernel.hinv3_i=kernel.hinv_i*kernel.hinv_i*kernel.hinv_i;kernel.hinv4_i=kernel.hinv3_i*kernel.hinv_i;
	//searchiter += 1;
	  //printf("h = %f, hinv = %f, hinv3 = %f, hinv4 = %f\n",kernel.h_i,kernel.hinv_i,kernel.hinv3_i,kernel.hinv4_i);
	if(mode == 1) {listindex++; if(listindex < NODELISTLENGTH) {startnode = DATAGET_NAME[target].NodeList[listindex]; if(startnode >= 0) {startnode = Nodes[startnode].u.d.nextnode; /* open it */}}} /* continue to open leaves if needed */
      }
    ...
    //printf("COllECTED NGBS\n");
    //if(ngbcount < 32){return 0;}
    return 0;
    /*Sort particles by distance*/
    struct str ngbsort[ngbcount];
    for(n = 0; n < ngbcount; n++){
      ngbsort[n].Rngb2 = rngb[n];
      ngbsort[n].index = ngblistsum[n];
    }
    qsort(ngbsort, ngbcount, sizeof(ngbsort[0]), cmp);
    //printf("SORTED\n");
	...
	if(((1 << local.Type) & (DM_SIDM)) && ((1 << P[j].Type) & (DM_SIDM)) && (local.ID != P[j].ID))
	  {
	    ...
	    if(ngbsort[n].Rngb2 / kernel.h_i > 1){printf("r > h!!\n");}
	    double u_n = ngbsort[n].Rngb2 * kernel.hinv_i;
	    //printf("u_n = %f ",u_n);
	    kernel_main(u_n, kernel.hinv3_i, kernel.hinv4_i, &kernel.wk_i, &kernel.dwk_i, 0);
	    double mj_wk = FLT(local.Mass * kernel.wk_i);
	    //if(mj_wk * dVmag * All.DM_InteractionCrossSection * local.dtime * units / 2 > 0){printf("mj_wk = %.8f",mj_wk * dVmag * All.DM_InteractionCrossSection * local.dtime * units / 2);}
	    prob += mj_wk * dVmag * All.DM_InteractionCrossSection * local.dtime * units / 2;	
	  }
      }
    //printf("ngbcount = %i, prob = %.20f, ID = %i\n",ngbcount,prob,local.ID);
    return 0;
	...
	for(n = 0; n < ngbcount; n++){
	  ...
	  if(((1 << local.Type) & (DM_SIDM)) && ((1 << P[j].Type) & (DM_SIDM)) && (local.ID != P[j].ID))
	    {
	      double u_l = ngbsort[n].Rngb2 * kernel.hinv_i;
	      for(k=0;k<3;k++)
		{
		  kernel.dv[k] = local.Vel[k] - P[j].Vel[k];
		  if(All.ComovingIntegrationOn) {kernel.dv[k] += All.cf_hubble_a * kernel.dp[k]/All.cf_a2inv;}
	      ...
	      double dVmag = sqrt(kernel.dv[0]*kernel.dv[0]+kernel.dv[1]*kernel.dv[1]+kernel.dv[2]*kernel.dv[2]) / All.cf_atime;
	      kernel_main(u_l, kernel.hinv3_i, kernel.hinv4_i, &kernel.wk_i, &kernel.dwk_i, 0);
	      prob_l += kernel.wk_i * P[j].Mass * dVmag * All.DM_InteractionCrossSection * local.dtime * units;
	      if(gsl_rng_uniform(random_generator) <= prob_l){SItarget = ngbsort[n].index;
		
		kernel.dp[0] = local.Pos[0] - P[target].Pos[0]; kernel.dp[1] = local.Pos[1] - P[target].Pos[1]; kernel.dp[2] = local.Pos[2] - P[target].Pos[2];
		for(k=0;k<3;k++)
		  {
		    kernel.dv[k] = local.Vel[k] - P[target].Vel[k];
		    if(All.ComovingIntegrationOn) {kernel.dv[k] += All.cf_hubble_a * kernel.dp[k]/All.cf_a2inv;}
		  }
		//printf("Scattering\n");
#ifdef DM_FUZZY
