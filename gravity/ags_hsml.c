...
/*if(mode == 0){
    ...
    if(mode == 0){
        memset(&SIDMtempInfo[target].ngbprob,0.0,sizeof(SIDMtempInfo[target].ngbprob));
    ...
    if(out->ngbcount > 0 && ((SIDMtempInfo[target].ngbcount + out->ngbcount) < 350)){
    if(SIDMtempInfo[target].ngbcount + out->ngbcount > 350){printf("NGB # = %i (too many, should crash)\n", out->ngbcount);}
...
#include "../system/code_block_xchange_perform_ops.h" /* this calls the large block of code which actually contains all the loops, MPI/OPENMP/Pthreads parallelization */
      ...
      /* do check on whether we have enough neighbors, and iterate for density-hsml solution */
		...
		/*#if defined(DM_SIDM)
		if(redo_particle == 0 && P[i].do_sidm == 0){
		  redo_particle = 1;
		  P[i].do_sidm = 1;
		}
		#endif*/
...
#endif
#endif
/*#if defined(DM_SIDM_RAND_TARGET)
  int ngblist_sum[300];
  double ngbprob[300];
#endif*/
}
...
#ifdef DM_SIDM
    if((1 << P[i].Type) & (DM_SIDM)) return 1; //(P[i].scatter == true)
#endif
#if defined(DM_FUZZY) || defined(FLAG_NOT_IN_PUBLIC_CODE)
